...
void solve() {
  ...
  // Loop to process all the children
  while (i <= j) {
    if (p[i] + p[j] <= x) {
      // If the lightest and heaviest child can share a gondola
      gondolas++;
      i++; // Move the lightest child
      j--; // Move the heaviest child
    } else {
      // If they can't share, the heaviest child goes alone
      gondolas++;
      j--; // Move only the heaviest child
    }
  }

  // Output the total number of gondolas
  cout << gondolas << endl;
}
