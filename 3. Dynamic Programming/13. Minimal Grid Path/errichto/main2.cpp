...
// const int INF = 1e9+5;
 ...
 void solve(){
    ...
    for(int diag = 2*n-2; diag>=0; diag--) {
        ...
        // vector<pair<pair<char, int>, int>> order; // coordinate compression
        vector<pair<int,  int>> order(k); // coordinate compression
        for(int i=0; i<k; i++) {
            auto [row, col] = v[i];
            // pair<char, int> state = make_pair(a[row][col] , min(hash[row+1][col], hash[row][col+1]));
            int state = (a[row][col] - 'a')*(n+2) + min(hash[row+1][col], hash[row][col+1]);
            order[i] = {state, row};
        }
