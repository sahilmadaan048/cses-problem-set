#include "bits/stdc++.h"
using namespace std;

using ll = long long; // safer than #define int long long

#define uint unsigned long long
#define vi vector<ll>
#define vvi vector<vi>
#define vb vector<bool>
#define vvb vector<vb>
#define fr(i, n) for (ll i = 0; i < (n); i++)
#define rep(i, a, n) for (ll i = (a); i <= (n); i++)
#define nl cout << "\n"
#define dbg(var) cout << #var << "=" << var << " "
#define all(v) v.begin(), v.end()
#define sz(v) (ll)(v.size())
#define srt(v) sort(v.begin(), v.end())         // sort
#define mxe(v) *max_element(v.begin(), v.end()) // find max element in vector
#define mne(v) *min_element(v.begin(), v.end()) // find min element in vector
#define unq(v) v.resize(distance(v.begin(), unique(v.begin(), v.end())))
#define bin(x, y) bitset<y>(x)

ll MOD = 1e9 + 7; // Hardcoded, directly change from here for functions!

void modadd(ll &a, ll b) { a = ((a % MOD) + (b % MOD)) % MOD; }
void modsub(ll &a, ll b) { a = ((a % MOD) - (b % MOD) + MOD) % MOD; }
void modmul(ll &a, ll b) { a = ((a % MOD) * (b % MOD)) % MOD; }

template <typename typC, typename typD>
istream &operator>>(istream &cin, pair<typC, typD> &a) { return cin >> a.first >> a.second; }
template <typename typC>
istream &operator>>(istream &cin, vector<typC> &a)
{
    for (auto &x : a)
        cin >> x;
    return cin;
}
template <typename typC, typename typD>
ostream &operator<<(ostream &cout, const pair<typC, typD> &a) { return cout << a.first << ' ' << a.second; }
template <typename typC, typename typD>
ostream &operator<<(ostream &cout, const vector<pair<typC, typD>> &a)
{
    for (auto &x : a)
        cout << x << '\n';
    return cout;
}
template <typename typC>
ostream &operator<<(ostream &cout, const vector<typC> &a)
{
    ll n = a.size();
    if (!n)
        return cout;
    cout << a[0];
    for (ll i = 1; i < n; i++)
        cout << ' ' << a[i];
    return cout;
}

void solve(const ll d[])
{
    ll a;
    cin >> a;
    cout << d[a] << "\n";
}

int main()
{
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    static ll d[1000005] = {}; // static -> global memory

    for (ll i = 1; i < 1000005; i++)
    {
        for (ll j = i; j < 1000005; j += i)
        {
            d[j]++;
        }
    }

    ll T;
    cin >> T;
    while (T--)
    {
        solve(d);
    }
    return 0;
}
